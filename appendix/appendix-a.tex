...
Every script behind the feature extraction pipeline, the alignment method implementations, and the diagnostic ablation study lives on the \texttt{labsrv7} cluster filesystem, organised under a single project directory rather than an external code repository. Raw extracted features for all six encoder combinations (Table~\ref{tab:app_sw}) sit there too, under \texttt{features\_100k/}, and aren't redistributed as part of this thesis --- simply because of their size, since $100{,}000$ objects across up to $1280$ dimensions per encoder adds up fast. The Objaverse dataset \cite{deitke2023objaverse} and its Cap3D captions \cite{luo2023scalable} are both publicly available from their original sources, and so are all the pre-trained encoder weights used in this work (PointNet, OpenCLIP ViT-bigG-14, BERT, RoBERTa), released by their respective original authors. The one exception is SparseConv, for which pre-trained weights simply weren't available at the time of writing (Section~\ref{subsec:3d_encoders}). Numerical results underlying every table in Chapters~\ref{ch:baseline}--\ref{ch:gw_alignment} are stored in the corresponding \texttt{results\_*.json} output files referenced in Table~\ref{tab:app_script_mapping}.